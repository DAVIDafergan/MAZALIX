...
// --- רכיב כפתור שיתוף (הגדרה כ-function למניעת שגיאות Build) ---
...
// --- רכיב כרטיס פרס (הגדרה כ-function למניעת שגיאות Build) ---
...
const Catalog: React.FC<{ store: any }> = ({ store }) => {
  ...
  const regularPrizes = useMemo(() => 
    clientPrizes.filter((p: Prize) => !p.isFeatured).sort((a, b) => a.order - b.order)
  , [clientPrizes]);
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-4 space-y-6 md:space-y-10">
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap justify-center gap-4 md:gap-8 text-gray-400 font-bold uppercase tracking-[0.3em] text-[8px] md:text-[10px] pt-4">
              <div className="flex items-center gap-2"><Calendar className="gold-text" size={12}/> {currentCampaign?.drawDate || 'TBD'}</div>
          ...
          <div className="relative h-[320px] md:h-[520px] w-full floating glass-card rounded-[2.5rem] md:rounded-[4.5rem] border-2 border-[#C2A353]/30 overflow-hidden shadow-[0_0_120px_rgba(194,163,83,0.2)] group">
                ...
                <div key={p.id} className={`absolute inset-0 transition-all duration-1000 ease-in-out ${isActive ? 'opacity-100 translate-x-0 scale-100 z-10' : 'opacity-0 translate-x-full scale-105 z-0'}`}>
                  ...
                  <div className="absolute bottom-12 left-12 right-12 md:bottom-24 md:left-24 md:right-24 flex flex-col md:flex-row md:items-end justify-between gap-12 z-20">
                    <div className="space-y-4 max-w-3xl">
                       ...
                       <p className="text-gray-200 text-sm md:text-2xl font-bold italic line-clamp-2 max-w-2xl leading-relaxed drop-shadow-lg">{isHE ? p.descriptionHE : p.descriptionEN}</p>
                       <div className="flex items-center gap-8 pt-4">
                          ...
                          <span className="text-4xl md:text-7xl font-black italic gold-text tracking-tighter drop-shadow-[0_5px_15px_rgba(0,0,0,0.5)]">₪{p.value?.toLocaleString() || 0}</span>
                       </div>
                    </div>
                    <div className="flex gap-5">
                        ...
                        {(currentCampaign?.donationUrl || campaign?.donationUrl) && (
                          <div className="flex flex-col gap-2">
                            <a href={currentCampaign.donationUrl || campaign.donationUrl} target="_blank" rel="noreferrer" className="px-12 md:px-20 h-16 md:h-24 luxury-gradient text-black font-black rounded-[2rem] flex items-center justify-center shadow-[0_20px_60px_rgba(194,163,83,0.4)] hover:scale-105 active:scale-95 transition-all text-sm md:text-xl uppercase italic tracking-tighter">{isHE ? 'להשתתפות עכשיו' : 'Enter Auction'}</a>
                            <a href={currentCampaign.donationUrl || campaign.donationUrl} target="_blank" rel="noreferrer" className="text-center text-[10px] md:text-xs font-black text-white/50 hover:text-white transition-colors uppercase tracking-widest underline italic">{isHE ? 'רכישה מהירה' : 'Quick Purchase'}</a>
                          </div>
                        )}
                    </div>
                  </div>
            ...
            <div className="absolute bottom-10 right-1/2 translate-x-1/2 flex gap-4 z-30">
              {featuredPrizes.map((_, idx) => (
                <div key={idx} className={`h-2.5 rounded-full transition-all duration-700 shadow-xl ${idx === featuredIndex ? 'w-16 bg-[#C2A353] shadow-[0_0_20px_#C2A353]' : 'w-2.5 bg-white/30'}`} />
              ))}
            </div>
          </div>
        </div>
            ...
            <section className="space-y-3 md:space-y-5 px-1 md:px-0">
              ...
              <div className="flex gap-3 overflow-x-auto pb-6 scrollbar-hide px-4">
                {clientPackages.map((pkg: Package) => {
                  ...
                  return (
                    <div key={pkg.id} style={{ borderTop: `4px solid ${pkg.color || '#C2A353'}` }} className="min-w-[180px] md:min-w-[260px] h-[220px] md:h-[280px] relative rounded-2xl md:rounded-[2rem] overflow-hidden glass-card shrink-0 shadow-lg group transition-all">
                      {pkg.image && <img src={pkg.image} className="absolute inset-0 w-full h-full object-cover opacity-30 group-hover:scale-110 transition-transform duration-[2s]" alt="" />}
            ...
            <section className="space-y-6 md:space-y-8 px-1 md:px-0">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 md:gap-8 px-4">
                {regularPrizes.map((p: Prize) => {
                  ...
                  if (p.isFullPage) {
                    return (
                      <div key={p.id} className="col-span-full relative h-[400px] md:h-[650px] rounded-[3rem] md:rounded-[4rem] overflow-hidden glass-card shadow-2xl group border border-white/5 animate-fade-in my-6">
                        <img src={p.media?.[0]?.url || ''} className="w-full h-full object-cover grayscale-[0.2] group-hover:grayscale-0 group-hover:scale-105 transition-all duration-[3s]" alt="" />
                  ...
                  return (
                    <PrizeCard key={p.id} activeClientId={activeClientId} prize={p} isHE={isHE} campaignName={currentCampaign?.nameHE || 'Mazalix'} donationUrl={currentCampaign?.donationUrl || campaign?.donationUrl} ticketCount={ticketCount} />
                  );
                })}
              </div>
            </section>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-10 animate-fade-in">
          ...
          <div className="relative w-full max-w-xl glass-card rounded-[2.5rem] border overflow-hidden shadow-2xl" style={{ borderColor: `${selectedPkg.color}40` }}>
             ...
             <div className="h-32 md:h-48 relative">
               <img src={selectedPkg.image || campaign?.banner || ''} className="w-full h-full object-cover opacity-50" alt="" />
               <div className="absolute inset-0 bg-gradient-to-t from-[#020617] to-transparent"></div>
             ...
             <div className="p-8 md:p-10 space-y-6 overflow-y-auto max-h-[60vh] scrollbar-hide">
                <div className="grid grid-cols-1 gap-3">
                   {Array.isArray(selectedPkg.rules) && selectedPkg.rules.map((rule: any, idx: number) => {
                     const prize = rule.prizeId === 'ALL' ? null : prizes.find((p: any) => p.id === rule.prizeId);
                     return (
                        <div key={idx} className="flex items-center justify-between p-4 bg-white/5 rounded-2xl border border-white/5 hover:bg-white/10 transition-all group">
                           <div className="flex items-center gap-4"><div className="w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center text-[#C2A353] overflow-hidden border border-white/5"> {rule.prizeId === 'ALL' ? <Layers size={20} /> : prize?.media?.[0]?.url ? <img src={prize.media[0].url} className="w-full h-full object-cover" alt="" /> : <Gift size={20} />}</div><div><p className="text-xs md:text-sm font-black italic leading-tight">{rule.prizeId === 'ALL' ? (isHE ? 'כל הפרסים בקטלוג' : 'All Collection Items') : (isHE ? prize?.titleHE : prize?.titleEN)}</p><p className="text-[8px] md:text-[10px] text-gray-500 font-black uppercase tracking-[0.2em]">{isHE ? 'חבילת מזל' : 'Luck Package'}</p></div></div>
                ...
                <a href={selectedPkg.joinLink || '#'} target="_blank" rel="noreferrer" className="block w-full text-center py-4 rounded-2xl text-black font-black text-sm uppercase tracking-widest shadow-xl transition-all hover:scale-[1.02] active:scale-95 italic" style={{ backgroundColor: selectedPkg.color || '#C2A353' }}>{isHE ? 'רכישת מסלול' : 'Acquire Now'} — ₪{selectedPkg.minAmount.toLocaleString()}</a>
             </div>
          </div>
        </div>
      )}
