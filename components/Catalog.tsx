...
function PrizeCard({ activeClientId, prize, isHE, campaignName, donationUrl, ticketCount }: any) {
  ...
  return (
    <div className="group relative rounded-[2.5rem] md:rounded-[3rem] p-3 md:p-5 bg-white/[0.02] hover:bg-white/[0.05] border border-white/5 hover:border-[#C2A353]/40 transition-all duration-700 animate-fade-in flex flex-col shadow-2xl overflow-hidden">
      {/* תאורת רקע עדינה ב-Hover */}
      <div className="absolute inset-0 bg-gradient-to-br from-[#C2A353]/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-700"></div>
      
      <div className="relative h-44 md:h-72 rounded-[1.5rem] md:rounded-[2.5rem] overflow-hidden mb-4 md:mb-6 shadow-2xl border border-white/5">
        <img src={prize?.media?.[0]?.url || ''} className="w-full h-full object-cover grayscale-[0.2] group-hover:grayscale-0 group-hover:scale-110 transition-all duration-[2s]" alt="" />
        ...
        <div className="absolute top-4 left-4 md:top-6 md:left-6 z-10 flex gap-3">
          <div className="px-4 py-1.5 bg-black/70 backdrop-blur-2xl rounded-2xl border border-white/10 flex items-center gap-2 shadow-2xl">
              <Ticket size={14} className="gold-text" />
              <span className="text-[10px] md:text-sm font-black italic text-white">{ticketCount?.toLocaleString() || 0}</span>
          </div>
          <PrizeShareButton activeClientId={activeClientId} prize={prize} isHE={isHE} campaignName={campaignName} />
        </div>

        {prize?.isFeatured && (
          <div className="absolute bottom-4 left-4 md:bottom-6 md:left-6 px-4 py-1.5 bg-[#C2A353] rounded-xl text-black font-black text-[9px] md:text-[11px] uppercase italic shadow-[0_0_20px_rgba(194,163,83,0.4)]">
            {isHE ? 'מומלץ' : 'FEATURED'}
          </div>
        )}
      </div>

      <div className="px-1 md:px-3 pb-2 space-y-4 md:space-y-6 flex-1 flex flex-col justify-between text-center relative z-10">
        <div className="space-y-2">
          <h3 className="text-base md:text-2xl font-black tracking-tight group-hover:gold-text transition-colors duration-700 leading-tight italic line-clamp-1">{isHE ? prize?.titleHE : prize?.titleEN}</h3>
          <p className="text-gray-400 text-[10px] md:text-sm leading-relaxed line-clamp-2 font-medium italic opacity-80">{isHE ? prize?.descriptionHE : prize?.descriptionEN}</p>
        </div>

        <div className="pt-2 flex flex-col items-center gap-4 md:gap-6">
          <div className="flex flex-col items-center gap-1">
            <span className="text-[8px] md:text-[10px] text-gray-500 font-black uppercase tracking-[0.2em]">
              {isHE ? 'בשיווי' : 'Estimated Value'}
            </span>
            <p className="text-xl md:text-3xl font-black italic leading-none gold-text tracking-tighter drop-shadow-lg">
          ...
          {donationUrl && (
            <div className="w-full flex flex-col gap-2">
              <a href={donationUrl} target="_blank" rel="noreferrer" className="w-full py-3 md:py-5 bg-white/5 border border-white/10 rounded-2xl md:rounded-[2rem] font-black text-[10px] md:text-[12px] uppercase tracking-[0.25em] hover:luxury-gradient hover:text-black hover:border-transparent hover:scale-[1.02] transition-all duration-500 text-center italic shadow-xl group/btn overflow-hidden relative">
  ...
  return (
    <div className="space-y-10 md:space-y-20 pb-20 max-w-[1600px] mx-auto px-4 md:px-8 animate-fade-in">
      ...
      <header className="relative h-[500px] md:h-[750px] rounded-[3rem] md:rounded-[4rem] overflow-hidden group shadow-[0_50px_100px_-20px_rgba(0,0,0,0.7)] border border-white/10 mx-auto w-full">
        ...
        {/* מונה כרטיסים צף */}
        <div className="absolute top-8 right-8 md:top-12 md:right-12 z-20 flex items-center gap-4 bg-black/40 backdrop-blur-3xl px-6 py-3 md:px-8 md:py-4 rounded-[2rem] border border-[#C2A353]/30 shadow-2xl animate-float">
           <Ticket size={24} className="gold-text" />
           <div className={`${isHE ? 'text-right' : 'text-left'}`}>
             <p className="text-xl md:text-3xl font-black tracking-tighter italic leading-none text-white">{clientTickets.length.toLocaleString()}</p>
             <p className="text-[8px] md:text-[10px] font-black uppercase text-gray-400 tracking-[0.2em] mt-1">{isHE ? 'כרטיסי מזל' : 'Luck Tokens'}</p>
           </div>
        </div>
