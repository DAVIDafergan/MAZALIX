...
const Catalog: React.FC<CatalogProps> = ({ store }) => {
  ...
  const clientPackages = useMemo(() => 
    activeClientId ? packages.filter((p: any) => p.clientId === activeClientId) : packages
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 px-4">
          {clients.length === 0 ? (
            <div className="col-span-full text-center py-20 opacity-50 italic">
               {isHE ? 'טוען קמפיינים...' : 'Loading campaigns...'}
            </div>
          ) : (
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-4 space-y-6 md:space-y-10">
          ...
          <div className="space-y-2">
            <h1 className="text-2xl md:text-6xl font-black tracking-tighter luxury-gradient bg-clip-text text-transparent italic leading-tight">
              {isHE ? currentCampaign.nameHE || currentClient?.displayName : currentCampaign.nameEN || currentClient?.displayName}
            </h1>
