...
const AdminDashboard: React.FC<AdminProps> = ({ store }) => {
  ...
  // תיקון קריטי סופי: לוגיקת בחירת מתנה במסלול - הבטחת ריבוי בחירות
  const handleTogglePrizeInPkg = (prizeId: string | 'ALL') => {
    if (prizeId === undefined || prizeId === null) return;
    
    setPkgForm(prev => {
      const currentRules = [...(prev.rules || [])];
      const targetId = String(prizeId);
      const existingIdx = currentRules.findIndex(r => String(r.prizeId) === targetId);
      
      if (existingIdx > -1) {
        return { ...prev, rules: currentRules.filter(r => String(r.prizeId) !== targetId) };
      } else {
        return { ...prev, rules: [...currentRules, { prizeId: targetId, count: 1 }] };
      }
    });
  };
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                    <div className="md:col-span-2 relative">
                      <input 
                        ...
                        onChange={e => setNewPrize(prev => ({...prev, titleHE: e.target.value}))} 
                      />
                      <button 
                    ...
                    <input placeholder={isHE ? 'שווי מוערך ₪' : 'Market Value ₪'} type="number" className="bg-white/10 p-2 rounded-lg text-xs font-bold outline-none border border-white/5 focus:border-[#C2A353]" value={newPrize.value || ''} onChange={e => setNewPrize(prev => ({...prev, value: Number(e.target.value)}))} />
                    ...
                    <textarea placeholder={isHE ? 'תיאור שיווקי יוקרתי (עברית)' : 'Luxury Description (HE)'} className="md:col-span-2 bg-white/10 p-2 rounded-lg text-xs min-h-[60px] outline-none border border-white/5 focus:border-[#C2A353]" value={newPrize.descriptionHE} onChange={e => setNewPrize(prev => ({...prev, descriptionHE: e.target.value}))} />
                    <textarea placeholder={isHE ? 'Luxury Description (English)' : 'Luxury Description (EN)'} className="md:col-span-2 bg-white/10 p-2 rounded-lg text-xs min-h-[60px] outline-none border border-white/5 focus:border-[#C2A353]" value={newPrize.descriptionEN} onChange={e => setNewPrize(prev => ({...prev, descriptionEN: e.target.value}))} />
              ...
              <div className="bg-white/5 p-4 rounded-xl border border-white/5 space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <div className="space-y-1">
                    ...
                    <input className="w-full bg-white/10 p-2 rounded-lg text-xs font-bold outline-none focus:border-[#C2A353]" value={pkgForm.nameHE} onChange={e => setPkgForm(prev => ({...prev, nameHE: e.target.value}))} />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[8px] font-black text-gray-500 uppercase">{isHE ? 'סכום' : 'Amount'}</label>
                    <input className="w-full bg-white/10 p-2 rounded-lg text-xs font-bold outline-none focus:border-[#C2A353]" type="number" value={pkgForm.minAmount || ''} onChange={e => setPkgForm(prev => ({...prev, minAmount: Number(e.target.value)}))} />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[8px] font-black text-gray-500 uppercase">{isHE ? 'תמונה' : 'Image'}</label>
                    <div className="flex gap-1">
                      <input placeholder={isHE ? 'URL תמונה' : 'Img URL'} className="flex-1 bg-white/10 p-2 rounded-lg text-[10px]" value={pkgForm.image} onChange={e => setPkgForm(prev => ({...prev, image: e.target.value}))} />
                  ...
                  <div className="space-y-1">
                    ...
                    <input placeholder={isHE ? 'לינק הצטרפות' : 'Join Link'} className="w-full bg-white/10 p-2 rounded-lg text-[10px]" value={pkgForm.joinLink} onChange={e => setPkgForm(prev => ({...prev, joinLink: e.target.value}))} />
                  </div>
                  <div className="space-y-1 md:col-span-2">
                    <label className="text-[8px] font-black text-gray-500 uppercase">{isHE ? 'צבע מסלול' : 'Route Color'}</label>
                    <div className="flex items-center gap-2">
                      <input type="color" className="w-10 h-10 rounded-lg bg-transparent cursor-pointer border-none" value={pkgForm.color} onChange={e => setPkgForm(prev => ({...prev, color: e.target.value}))} />
                      <input type="text" className="flex-1 bg-white/10 p-2 rounded-lg text-[10px] outline-none" value={pkgForm.color} onChange={e => setPkgForm(prev => ({...prev, color: e.target.value}))} />
                    </div>
                  </div>
                </div>
                    ...
                    return (
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div className="min-w-0">
                                ...
                                {/* הצגת מספר טלפון */}
                                <p className="text-[8px] text-gray-500 font-bold">{d.phone}</p>
                                <div className="flex items-center gap-1">
                                    <Activity size={8} className={assignedPkg ? 'text-green-500' : 'text-red-500'} />
                                    <p className={`text-[7px] font-black uppercase italic ${assignedPkg ? 'text-gray-400' : 'text-red-500'}`}>{assignedPkg ? (isHE ? assignedPkg.nameHE : assignedPkg.nameEN) : (isHE ? 'טרם שויך' : 'UNMAPPED')}</p>
                                </div>
                            </div>
                        </div>
                        <div className="flex items-center gap-4">
