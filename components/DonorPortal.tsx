...
import { Smartphone, Ticket as TicketIcon, User, LogOut, ChevronRight, Activity, Award, Gift, Sparkles, ShieldCheck } from 'lucide-react';
...
const DonorPortal: React.FC<PortalProps> = ({ store }) => {
  const { donors, tickets, prizes, lang } = store;
  ...
  const handleLogin = () => {
    const found = donors.find((d: Donor) => d.phone === phone);
    if (found) {
      setLoggedInDonor(found);
      setError('');
    } else {
      setError(isHE ? 'תורם לא נמצא.' : 'Donor not found.');
    }
  };
